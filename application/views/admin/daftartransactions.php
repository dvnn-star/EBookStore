        ...
        <main class="flex-1 flex flex-col">
            ...
            <header class="h-20 bg-white/80 backdrop-blur-md border-b border-slate-200/60 flex items-center justify-between px-10 sticky top-0 z-10">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="<?= base_url('admin/export_transaksi') ?>" class="px-5 py-2.5 text-sm font-bold text-slate-700 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 transition-all shadow-sm flex items-center gap-2">
                                    ...
                                    <?php foreach ($transactions as $tr): ?>
                                        <tr class="group hover:bg-slate-50/80 transition-all">
                                            ...
                                            <td class="px-8 py-5">
                                                ...
                                                <p class="text-[10px] text-slate-400 uppercase tracking-tight"><?= html_escape($tr->payment_method ?? 'Transfer Bank') ?></p>
                                            </td>
                                            ...
                                            <td class="px-8 py-5">
                                                <div class="flex items-center justify-center gap-2">
                                                    <!-- Tombol Detail -->
                                                    <a href="<?= base_url('admin/detail_transaksi/' . $tr->kode_transaksi) ?>" class="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-all" title="Lihat Detail">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg>
                                                    </a>
                                                    <!-- Tombol Cetak Invoice -->
                                                    <a href="<?= base_url('admin/cetak_invoice/' . $tr->kode_transaksi) ?>" target="_blank" class="p-2 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition-all" title="Cetak Invoice">
