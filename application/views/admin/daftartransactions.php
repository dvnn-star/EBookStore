        ...
        <main class="flex-1 flex flex-col">
            <header class="h-20 bg-[#FFFFFF] border-b border-[#2C3E50]/20 flex items-center justify-between px-10 sticky top-0 z-10 shadow-sm">
                ...
                <div class="flex items-center gap-4">
                    <a href="<?= base_url('export_csv') ?>" class="px-5 py-2.5 text-sm font-bold text-white bg-[#005B52] rounded-xl hover:bg-[#00443d] transition-all shadow-md shadow-[#005B52]/10 flex items-center gap-2 active:scale-95">
            ...
            <section class="p-10 space-y-8">
                <div class="bg-[#FFFFFF] border border-[#2C3E50]/20 rounded-3xl shadow-md overflow-hidden">
                    <div class="px-8 py-6 border-b border-[#2C3E50]/20 bg-[#F8F9FA] flex justify-between items-center">
                        ...
                        <select class="text-sm border-[#2C3E50]/30 rounded-lg text-[#2C3E50] font-semibold focus:ring-2 focus:ring-[#005B52]/20 focus:border-[#005B52] py-2 px-3 bg-[#FFFFFF] border shadow-sm outline-none">
                        ...
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-[#2C3E50]/10">
                                <?php if (!empty($transactions)): ?>
                                    <?php foreach ($transactions as $tr): ?>
                                        <tr class="group hover:bg-[#F8F9FA] transition-all">
                                            ...
                                            <td class="px-8 py-5 text-center">
                                                <?php
                                                $status_classes = ['success' => 'bg-emerald-50 text-emerald-700 border-emerald-200', 'pending' => 'bg-amber-50 text-amber-700 border-amber-200', 'failed' => 'bg-rose-50 text-rose-700 border-rose-200', 'expired' => 'bg-slate-100 text-slate-600 border-slate-200'];
                                                $statusKey = strtolower($tr->status);
                                                $class = $status_classes[$statusKey] ?? 'bg-slate-100 text-slate-600 border-slate-200';
                                                ?>
                                                <span class="inline-flex items-center px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border <?= $class ?>"><?= html_escape($tr->status) ?></span>
                                            </td>
                                            <td class="px-8 py-5">
                                                <div class="flex items-center justify-center gap-3">
                                                    ...
                                                    <?php if ($statusKey === 'success'): ?>
                                                        <a href="<?= base_url('DaftarTransactions/print/' . $tr->kode_transaksi) ?>" target="_blank" class="bg-[#2C3E50]/10 hover:bg-[#2C3E50]/20 text-[#2C3E50] px-4 py-2 text-xs font-bold rounded-xl transition-all flex items-center gap-1.5" title="Cetak Invoice">
                                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5"><path stroke-linecap="round" stroke-linejoin="round" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" /></svg>
                                                            Print Receipt
                                                        </a>
                                                    <?php else: ?>
                                                        <a href="<?= base_url('DaftarTransactions/update_status/' . $tr->kode_transaksi . '/success') ?>" onclick="return confirm('Apakah Anda yakin ingin MENGONFIRMASI pembayaran ini?')" class="bg-[#005B52] rounded-xl hover:bg-[#00443d] px-4 py-2 text-white text-xs font-bold transition-all shadow-md active:scale-95 inline-block">
                                                            Konfirmasi Success
                                                        </a>
                                                    <?php endif; ?>
                                                </div>
                                    ...
                                    <?php endforeach; ?>
                                <?php else: ?>
                                    <tr><td colspan="5" class="px-8 py-20 text-center font-bold text-[#2C3E50]/60 italic text-sm">Belum ada transaksi.</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
